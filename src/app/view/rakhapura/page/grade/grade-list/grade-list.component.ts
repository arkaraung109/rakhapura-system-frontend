...
export class GradeListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.gradeService.fetchPageSegment(this.currentPage).subscribe({
  ...
  sortData(sort: Sort) {
    ...
    this.sortedData = data.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'abbreviate':
          return this.compare(a.abbreviate, b.abbreviate, isAsc);
        default:
          return 0;
      }
    });
  ...
  search() {
    ...
    this.gradeService.fetchPageSegmentBySearching(this.currentPage, PaginationOrder.DESC, this.keyword).subscribe({
      next: (res: PaginationResponse) => {
        this.setDataInCurrentPage(res);
  ...
  authorize(id:number, authorizedUserId: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.gradeService.authorize(id, authorizedUserId).subscribe({
          next: (res: ApiResponse) => {
            if(res.status == HttpCode.OK) {
