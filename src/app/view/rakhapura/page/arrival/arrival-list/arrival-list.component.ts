...
export class ArrivalListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.arrivalService.fetchPageSegment(this.currentPage).subscribe({
  ...
  search() {
    ...
    if(this.searchedExamTitle == 0 && this.searchedAcademicYear == 0 && this.searchedGrade == 0 && this.searchedClass === 'All' && this.keyword === '') {
      ...
      this.arrivalService.fetchPageSegmentBySearching(this.currentPage, PaginationOrder.DESC, this.searchedExamTitle, this.searchedAcademicYear, this.searchedGrade, this.searchedClass, this.keyword).subscribe({
        next: (res: PaginationResponse) => {
          this.setDataInCurrentPage(res);
          this.sort.sort({ id: 'id', start: 'desc', disableClear: false });
        },
  ...
  viewAttendence(id: number) {
    this.router.navigate(['/app/attendence/detail'], {
      queryParams: {
          id: id
      },
