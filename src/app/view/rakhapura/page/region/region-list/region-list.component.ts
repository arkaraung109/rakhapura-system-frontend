...
export class RegionListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.keyword === '') {
      this.regionService.fetchPageSegment(this.currentPage).subscribe({
      ...
      this.submitted = true;
      this.regionService.fetchPageSegmentBySearching(this.currentPage, PaginationOrder.DESC, this.keyword).subscribe({
        next: (res: PaginationResponse) => {
          this.setDataInCurrentPage(res);
  ...
  authorize(id: number, authorizedUserId: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        this.regionService.authorize(id, authorizedUserId).subscribe({
          next: (res: ApiResponse) => {
            if(res.status == HttpCode.OK) {
