...
export class ProfileEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.oldApplicationUser = this.userService.fetchUserProfileInfo();
    ...
    if (this.userInfo.role.name !== UserPermission.ADMIN) {
      this.form.get('loginUserName')!.disable();
    }
  ...
  update() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        requestBody.password = this.form.get('newPassword')!.value.trim();
        requestBody.oldPassword = this.form.get('oldPassword')!.value.trim();
        requestBody.activeStatus = this.oldApplicationUser.activeStatus;
        requestBody.role.id = this.form.get('role')!.value;

        this.userService.update(requestBody, this.oldApplicationUser.id).subscribe({
          next: (res: ApiResponse) => {
            if (res.status == HttpStatusCode.Ok) {
              localStorage.setItem("status", "updated");
              this.userService.fetchById(this.oldApplicationUser.id).subscribe(data => {
                localStorage.removeItem("userInfo");
                this.userService.storeUserProfileInfo(data);
          ...
          error: (err) => {
            if (err.status == HttpStatusCode.Unauthorized) {
              ...
              if (err.error.message === "disabled object cannot be updated.") {
                this.toastrService.warning("You cannot update this.", "Disabled Account");
              } else if (err.error.message === "old password is wrong.") {
                this.toastrService.warning("Please enter correct old password.", "Wrong Old Password");
              }
