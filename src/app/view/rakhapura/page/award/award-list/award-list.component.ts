...
export class AwardListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.awardService.fetchPageSegmentBySearching(this.currentPage, PaginationOrder.DESC, this.keyword).subscribe({
  ...
  sortData(sort: Sort) {
    ...
    this.sortedData = data.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'award':
          return this.compare(a.award, b.award, isAsc);
        case 'description':
  ...
  edit(id: number) {
    this.router.navigate(['/app/award/edit'], {
      queryParams: {
        id: id,
  ...
  exportToExcel() {
    if(this.sortedData.length == 0) {
      ...
      this.awardService.exportToExcel(this.keyword).subscribe({
        next: (response) => {
          let file = new Blob([response], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
          let filename = 'awarded_student_' + format(new Date(), 'dd-MM-yyyy HH:mm:ss') + '.xlsx';
